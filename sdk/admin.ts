...
import { createMiniHubSDK, PackageConfig, DEFAULT_CLOCK_ID } from './minihub.js';
 ...
 * SDK'yı başlatır
 */
function initializeSDK(config: Config) {
...
configCmd
  ...
  .action((options) => {
    ...
    if (options.package) config.packageId = options.package;
    if (options.jobBoard) config.jobBoardId = options.jobBoard;
    if (options.userRegistry) config.userRegistryId = options.userRegistry;
    if (options.employerRegistry) config.employerRegistryId = options.employerRegistry;
    if (options.clock) config.clockId = options.clock;
...
exportCmd
  ...
  .action(async (options) => {
    ...
    try {
      ...
      // Dosyaları yaz
      fs.writeFileSync(
        path.join(options.dir, 'jobs.json'),
        JSON.stringify(jobs, null, 2)
      );
      
      fs.writeFileSync(
        path.join(options.dir, 'users.json'),
        JSON.stringify(users, null, 2)
      );
      
      fs.writeFileSync(
        path.join(options.dir, 'employers.json'),
        JSON.stringify(employers, null, 2)
      );
      
      fs.writeFileSync(
        path.join(options.dir, 'stats.json'),
      ...
      fs.writeFileSync(
        path.join(options.dir, 'report.json'),
